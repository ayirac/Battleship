...
private:
	...
	unsigned y_correction_;
public:
	ButtonTexture();
	// Create a textured button with text centered at the middle
	ButtonTexture(sf::Vector2f pos, const sf::Vector2f& size, const unsigned& char_size, const std::string& text, const sf::Font& font, sf::Texture* normal_tex, sf::Texture* press_tex, 
		sf::Color default_color, sf::Color hover_color, sf::Color press_color, unsigned state, unsigned y_correction = 0);
	// Create a texture button with no text
	ButtonTexture(sf::Vector2f pos, const sf::Vector2f& size, const std::string& text, sf::Texture* normal_tex, sf::Texture* press_tex, unsigned state);
	// Updates the button state
	void update(sf::Vector2f& mouse_pos, sf::RenderWindow& window);
